...
MODELS_AVAILABLE = (
    EXPORTED_DIR.exists() and any(EXPORTED_DIR.iterdir())
...
class TestRealModels:
    ...
    def test_onnx_detector_initialization(self):
        ...
        detector = MVTecONNXDetector(
            exported_dir=str(EXPORTED_DIR), class_name=first_class
        )
    ...
    def test_auto_detector_initialization(self):
        ...
        detector = AutoAnomalyDetector(
            exported_dir=str(EXPORTED_DIR), retrieval_k=5, anomaly_k=1
        )
    ...
    def test_full_detection_pipeline(self, sample_rgb_image):
        ...
        with patch.dict(os.environ, {"EXPORTED_DIR": str(EXPORTED_DIR)}):
            ...
            detect_response = client.post(
                "/detect", files={"file": ("test.png", image_bytes, "image/png")}
            )
...
class TestPerformance:
    ...
    def test_batch_prediction(self, sample_rgb_image):
        ...
        for i, result in enumerate(results):
            ...
            assert (
                "class_confidence" in result
            ), f"Resultado {i}: falta class_confidence"
            assert "anomaly_map" in result, f"Resultado {i}: falta anomaly_map"

            # Verificar tipos de datos
            assert isinstance(
                result["class_name"], str
            ), f"Resultado {i}: class_name no es string"
            ...
            if result.get("warning") is None:
                ...
                ), f"Resultado {i}: anomaly_score es None sin warning"
                assert (
                    result["anomaly_score"] >= 0
                ), f"Resultado {i}: anomaly_score negativo"
                assert (
                    0 <= result["class_confidence"] <= 1
                ), f"Resultado {i}: class_confidence fuera de [0,1]"
                assert result["anomaly_map"].shape == (
                    37,
                    37,
                ), f"Resultado {i}: shape incorrecto de anomaly_map"
