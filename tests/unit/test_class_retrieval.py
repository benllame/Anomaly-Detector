...
class TestClassRetriever:
    ...
    def test_preprocess_output_shape(self, sample_rgb_image):
        ...
        with patch('onnxruntime.InferenceSession') as mock_session_class:
            ...
            mock_session.get_inputs.return_value = [MagicMock(name='pixel_values', shape=[1, 3, 518, 518])]
            mock_session.get_outputs.return_value = [MagicMock(name='embeddings', shape=[1, 1369, 1024])]
            mock_session_class.return_value = mock_session
            
            with patch('deployment.class_retrieval.ClassRetriever._load_class_cls_tokens'):
                ...
                with patch('os.path.exists', return_value=True):
                    ...
                    retriever.class_tokens = {'bottle': np.random.randn(100, 1024).astype(np.float32)}
    ...
    def test_identify_class_returns_tuple(self, sample_rgb_image):
        ...
        with patch('onnxruntime.InferenceSession') as mock_session_class:
            ...
            mock_session.get_inputs.return_value = [MagicMock(name='pixel_values', shape=[1, 3, 518, 518])]
            mock_session.get_outputs.return_value = [MagicMock(name='embeddings', shape=[1, 1369, 1024])]
            mock_session.run.return_value = [np.random.randn(1, 1369, 1024).astype(np.float32)]
            mock_session_class.return_value = mock_session
            ...
            with patch('deployment.class_retrieval.ClassRetriever._load_class_cls_tokens'):
                ...
                with patch('os.path.exists', return_value=True):
                    ...
                    retriever.all_cls_tokens = retriever.all_tokens  # identify_class uses this name
                    retriever.token_labels = ['bottle'] * 50 + ['cable'] * 50
                    retriever.all_labels = retriever.token_labels  # identify_class uses this name
                    retriever.k = 5
                    retriever.use_patch_model = True
                    
                    result = retriever.identify_class(sample_rgb_image, return_all_scores=True)
...
class TestAutoAnomalyDetector:
    ...
    def test_compute_anomaly_map_shape(self, sample_patch_embeddings, sample_memory_bank):
    ...
    def test_compute_anomaly_map_valid_output(self, sample_patch_embeddings, sample_memory_bank):
    ...
    def test_predict_returns_dict(self, sample_rgb_image):
        ...
        with patch.object(AutoAnomalyDetector, '__init__', lambda self, **kwargs: None):
            ...
            detector.retriever.identify_class.return_value = (
                'bottle', 0.95, {'bottle': 0.95, 'cable': 0.05}
            )
            
            detector.session = MagicMock()
            detector.session.run.return_value = [np.random.randn(1, 1369, 1024).astype(np.float32)]
            ...
            # Mock de memory bank
            detector._get_memory_bank = MagicMock(return_value=np.random.randn(3000, 1024).astype(np.float32))
            
            # Mock preprocess
            detector.preprocess = MagicMock(return_value=np.random.randn(1, 3, 518, 518).astype(np.float32))
...
class TestComponentIntegration:
    ...
    def test_end_to_end_prediction_flow(self, sample_rgb_image):
        ...
        with patch.object(AutoAnomalyDetector, '__init__', lambda self, **kwargs: None):
            ...
            detector.retriever = MagicMock()
            detector.retriever.identify_class.return_value = (
                'bottle', 0.9, {'bottle': 0.9, 'cable': 0.1}
            )
...
class TestErrorHandling:
    ...
    def test_low_confidence_warning(self, sample_rgb_image):
        ...
        with patch.object(AutoAnomalyDetector, '__init__', lambda self, **kwargs: None):
            ...
            detector.retriever = MagicMock()
            detector.retriever.identify_class.return_value = (
                'bottle', low_confidence, {'bottle': 0.2, 'cable': 0.15}
            )
            ...
            assert 'warning' in result, "Debería generar warning cuando confianza < min_confidence"
            assert 'Confianza baja' in result['warning'], f"Warning incorrecto: {result.get('warning')}"
            assert f"{low_confidence:.2f}" in result['warning'], "Warning debe incluir el valor de confianza"
            
            # Verificar que no se calculó anomaly cuando la confianza es baja
            assert result['anomaly_score'] is None, "No debería calcular anomaly_score con baja confianza"
            assert result['anomaly_map'] is None, "No debería calcular anomaly_map con baja confianza"
