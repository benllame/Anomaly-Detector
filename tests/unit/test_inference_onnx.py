...
class TestPreprocessing:
    ...
    def test_preprocess_output_shape(self, sample_rgb_image, mock_onnx_session, configured_detector_dir):
        ...
        with patch('onnxruntime.InferenceSession', return_value=mock_onnx_session):
            with patch('numpy.load') as mock_np_load:
                mock_np_load.return_value = np.random.randn(3000, 1024).astype(np.float32)
                
                from deployment.inference_onnx import MVTecONNXDetector
                
                detector = MVTecONNXDetector(
                    exported_dir=str(configured_detector_dir),
                    class_name="bottle"
                )
    ...
    def test_preprocess_dtype(self, sample_rgb_image, mock_onnx_session, configured_detector_dir):
...
class TestAnomalyMapNormalization:
    ...
    def test_normalize_anomaly_map_minmax(self, sample_anomaly_map, mock_onnx_session, configured_detector_dir):
        ...
        with patch('onnxruntime.InferenceSession', return_value=mock_onnx_session):
            with patch('numpy.load') as mock_np_load:
                ...
                normalized = detector.normalize_anomaly_map(sample_anomaly_map, method='minmax')
    ...
    def test_normalize_constant_map(self, mock_onnx_session, configured_detector_dir):
        ...
        with patch('onnxruntime.InferenceSession', return_value=mock_onnx_session):
            with patch('numpy.load') as mock_np_load:
                ...
                normalized = detector.normalize_anomaly_map(constant_map, method='minmax')
...
class TestUpsampling:
    ...
    def test_upsample_anomaly_map(self, sample_anomaly_map, mock_onnx_session, configured_detector_dir):
...
class TestEvaluation:
    ...
    def test_evaluate_perfect_prediction(self, mock_onnx_session, configured_detector_dir):
