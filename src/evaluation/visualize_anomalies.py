...
class AnomalyVisualizer:
    ...
    def _get_detector(self, category: str) -> MemoryBankDetector:
        ...
        if category not in self._memory_banks:
            ...
            good_images = load_images_from_folder(good_path, n_images=self.n_good_images)
    ...
    def visualize_single_image(
        ...
        ax4.set_title(f"Predicción (τ={self.threshold})\n({n_pred:,} píxeles)", fontsize=12, fontweight='bold')
        ax4.axis('off')
    ...
    def visualize_anomaly_type(
        ...
        for i, (test_img, gt_mask, filename) in enumerate(test_data):
            ...
            # Ruta de guardado
            save_path = os.path.join(output_subdir, f"{filename.replace('.png', '_viz.png')}")
    ...
    def generate_summary_grid(
        ...
        for idx, anomaly_type in enumerate(anomaly_types):
            ...
            ax2.imshow(test_img)
            ax2.imshow(amap_norm, cmap='jet', alpha=0.5,
        ...
        fig.suptitle(
            f"Resumen de anomalías: {category.upper()}",
            fontsize=16, fontweight='bold'
        )
        
        # Leyenda
        fig.text(0.5, 0.02, "🟢 True Positive | 🔴 False Negative | 🔵 False Positive",
...
Ejemplos:
  ...
  python visualize_anomalies.py --category bottle --summary-grid
    ...
    parser.add_argument(
        '--all-categories',
        action='store_true',
        help='Visualizar todas las categorías'
    )
    parser.add_argument(
        '--n-samples',
        type=int,
    ...
    parser.add_argument(
        '--show',
        action='store_true',
        help='Mostrar visualizaciones en pantalla'
    )
    parser.add_argument(
        '--no-save',
        action='store_true',
        help='No guardar visualizaciones'
    )
