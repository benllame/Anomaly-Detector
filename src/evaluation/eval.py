...
class MVTecDatasetEvaluator:
    ...
    def __init__(
        ...
        print(f"🔍 Encontradas {len(self.available_categories)} categorías en {dataset_path}")
    ...
    def evaluate_category(
        ...
        if save_curves and len(all_anomaly_maps) > 0:
            ...
            curves_dir = visualization_dir or os.path.join('.', 'visualizations', category)
            ...
            curve_visualizer.export_curve_data(curves_dict, curves_data_path, format='json')
    ...
    def _print_final_summary(self, results: dict):
        ...
        print("-" * 80)
        print(f"{'Categoría':<15} {'IoU':<10} {'Dice':<10} {'F1':<10} {'Precision':<10} {'Recall':<10} {'AU-PRO':<10}")
        print("-" * 80)
    ...
    def generate_report(
        ...
        report.append(f"- **Imágenes 'good' para Memory Bank:** {config.get('n_good_images', 'Todas')}")
        report.append("")
...
class CurveVisualizer:
    ...
    def plot_curve(
        ...
        if curve_type == 'roc':
            ...
            # Add FPR limit line
            ax.axvline(x=curve_data['fpr_limit'], color='gray', linestyle='--', alpha=0.5)
            ...
            ax.scatter([best_t], [best_iou], color='red', s=100, zorder=5, 
            ...
            with open(output_path, 'w', newline='', encoding='utf-8') as f:
                ...
                for curve_name, curve_data in curves_dict.items():
                    ...
                    if curve_name == 'roc':
                        ...
                        for t, iou in zip(
                            curve_data['thresholds'],
                            curve_data['iou_values']
                        ):
                            writer.writerow([t, iou])
                        writer.writerow([f'# AU-IoU: {curve_data["au_iou"]:.4f}'])
                        writer.writerow([f'# Best: threshold={curve_data["best_threshold"]:.4f}, IoU={curve_data["best_iou"]:.4f}'])
    ...
    def generate_evaluation_report(
        ...
        for curve_name in ['roc', 'pr', 'pro', 'iou']:
            if curve_name in curves_dict:
                curve_path = os.path.join(output_dir, f"{prefix}{name_suffix}_{curve_name}.png")
        ...
        summary = {
            ...
            'metrics': {
                ...
                'Best Threshold': curves_dict.get('iou', {}).get('best_threshold', None)
            }
        }
        
        print(f"\n📈 Evaluation Report Generated:")
        print(f"   AU-ROC: {summary['metrics']['AU-ROC']:.4f}" if summary['metrics']['AU-ROC'] else "")
        print(f"   AU-PR:  {summary['metrics']['AU-PR']:.4f}" if summary['metrics']['AU-PR'] else "")
        print(f"   AU-PRO: {summary['metrics']['AU-PRO']:.4f}" if summary['metrics']['AU-PRO'] else "")
        print(f"   AU-IoU: {summary['metrics']['AU-IoU']:.4f}" if summary['metrics']['AU-IoU'] else "")
        print(f"   Best IoU: {summary['metrics']['Best IoU']:.4f} @ τ={summary['metrics']['Best Threshold']:.2f}" if summary['metrics']['Best IoU'] else "")
...
Ejemplos:
  ...
  python eval.py --mode full_evaluation --categories bottle cable carpet
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Número de vecinos más cercanos para k-NN'
    )
    parser.add_argument(
        '--threshold',
        type=float,
    ...
    # =========================================================================
        ...
        for anomaly_type, anom_data in results['anomaly_results'].items():
            s = anom_data['summary']
            print(f"  {anomaly_type:<20} {s.get('IoU', 0):<10.4f} "
