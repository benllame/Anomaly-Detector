...
class MVTecONNXDetector:
    ...
    def predict(
        self, image: Image.Image, smooth_sigma: float = 0.8, return_raw: bool = False
    ...
    def normalize_anomaly_map(
        self, anomaly_map: np.ndarray, method: str = "minmax"
    ) -> np.ndarray:
        """
    ...
    def evaluate(
        self, anomaly_map: np.ndarray, gt_mask: np.ndarray, threshold: float = 0.5
    ) -> Dict[str, float]:
        """
...
def benchmark_inference(
    detector: MVTecONNXDetector, image: Image.Image, n_runs: int = 100, warmup: int = 10
) -> Dict[str, float]:
    """
...
def evaluate_class(
    ...
    for defect_type in defect_types:
        ...
        test_images = sorted(
            [
                f
                for f in os.listdir(defect_test_path)
        ...
        if metrics_list:
            ...
            if verbose:
                print(
                    f"      IoU: {avg_metrics['IoU']:.3f}, F1: {avg_metrics['F1']:.3f}, "
    ...
    if all_metrics:
        ...
        if verbose:
            print(
                f"   PROMEDIO: IoU: {global_avg['IoU']:.3f}, F1: {global_avg['F1']:.3f}"
            )
...
def main():
    ...
    parser.add_argument("--image_path", type=str, help="Ruta a la imagen de prueba")
    ...
    parser.add_argument(
        "--threshold", type=float, default=0.5, help="Umbral de anomalía [0, 1]"
    )

    parser.add_argument(
        "--benchmark", action="store_true", help="Ejecutar benchmark de velocidad"
    )
    ...
    parser.add_argument(
        "--list_classes", action="store_true", help="Listar clases disponibles y salir"
    )
    ...
    # Cargar detector
    detector = MVTecONNXDetector(
        exported_dir=args.exported_dir, class_name=args.class_name, k=args.k
    )
    ...
    if args.image_path:
        ...
        print(
            f"   Es anomalía: {'Sí' if score > args.threshold else 'No'} (τ={args.threshold})"
        )
        ...
        if args.output:
            ...
            amap_upsampled = detector.upsample_anomaly_map(
                amap_norm, (image.height, image.width)
            )
            axes[2].imshow(image)
