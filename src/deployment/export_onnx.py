...
def get_test_images_path(data_root: str, class_name: str, defect_type: str = None) -> str:
...
def get_ground_truth_path(data_root: str, class_name: str, defect_type: str = None) -> str:
...
def list_defect_types(data_root: str, class_name: str) -> List[str]:
    ...
    if os.path.exists(test_path):
        return [d for d in os.listdir(test_path) 
...
def export_memory_bank(
    ...
    mb_metadata = {
        ...
        'hidden_dim': memory_bank_np.shape[1] if len(memory_bank_np.shape) > 1 else None,
        **(metadata or {})
    }
...
def verify_onnx_export(
    ...
    onnx_output = ort_session.run(
        None, 
        {'pixel_values': input_tensor.numpy()}
    )[0]
...
def export_for_class(
    ...
    train_path = get_train_images_path(data_root, class_name)
    n_train_images = len([f for f in os.listdir(train_path) 
...
Ejemplos:
  ...
  python export_onnx.py \\
    ...
    parser.add_argument(
        '--verify',
        action='store_true',
        help='Verificar exportación ONNX'
    )
