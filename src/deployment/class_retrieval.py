...
class ClassRetriever:
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "ONNX Runtime no instalado. pip install onnxruntime"
            )
    ...
    def _load_class_cls_tokens(self):
        ...
        if self.all_cls_tokens:
            ...
            raise ValueError("No se encontraron CLS tokens. Ejecuta export con --export_cls")
...
class AutoAnomalyDetector:
    ...
    def compute_anomaly_map(
        ...
        topk_indices = np.argpartition(sim_matrix, -self.anomaly_k, axis=1)[:, -self.anomaly_k:]
    ...
    def predict(
        ...
        if class_confidence < self.min_confidence:
            result['warning'] = f"Confianza baja: {class_confidence:.2f} < {self.min_confidence}"
    ...
    def visualize(
        ...
        if result.get('anomaly_map') is not None:
            ...
            axes[2].axis('off')
            axes[3].text(0.5, 0.5, result.get('warning', 'Error'), 
...
def main():
    ...
    parser.add_argument('--exported_dir', type=str, required=True,
    ...
    parser.add_argument('--retrieval_k', type=int, default=5,
                        help='K para clasificación')
    
    parser.add_argument('--anomaly_k', type=int, default=1,
                        help='K para detección de anomalías')
    
    parser.add_argument('--output', type=str,
                        help='Ruta para guardar visualización')
    
    parser.add_argument('--force_class', type=str,
                        help='Forzar clase específica')
